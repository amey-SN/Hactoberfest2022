...
def trackMultipleObjects():
    ...
    rectangleGreen = (0, 255, 0)
    rectangleRed = (0, 0, 255)
    frameCounter = 0
    currentCarID = 0
    fps = 0
    ...
    out = cv2.VideoWriter('outNew.mp4', cv2.VideoWriter_fourcc('M','J','P','G'), 10, (WIDTH, HEIGHT))
                ...
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    ...
                    if speed[i] != None and y1 >= 180:
                        if speed[i]>=speedLimit:
                            ...
                            cv2.rectangle(resultImage, (t_x, t_y), (t_x + t_w, t_y + t_h), rectangleRed, 4)
                        else:
                             cv2.putText(resultImage, str(int(speed[i])) + "km/h", (int(x1 + w1/2), int(y1-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0) ,2)
                             cv2.rectangle(resultImage, (t_x, t_y), (t_x + t_w, t_y + t_h), rectangleGreen, 4)
